...
##      uses my own cauchi algorithm to extract features and align the images ##
##############################################################################
...
try:
    while True:
        ...
        mask = np.where((uv_resized > clipping_distance_in_meters) | (uv_resized <= 0.0), 0.0, alphaValue)   #This could be optimized  #| bitwise OR (not AND)
        ...
        #  FEATURE EXTRACTION BY CANNY & SHI-TOMASI
        ...
        uvGray = cv.bilateralFilter(uvGray, 7, 20, 20)   #7, 50, 50 original
        uvGray = cv.Canny(uvGray, 60, 150)    #original 60, 120
        rgbGray = cv.bilateralFilter(rgbGray, 7, 20, 20)
        rgbGray = cv.Canny(rgbGray, 60, 150)    #original 60, 120

        masko =  cv.convertScaleAbs(mask, alpha=alphaValue)
        ...
        cv.imshow('debug_1', uv_image)
        cv.imshow('debug_2', color_image)
