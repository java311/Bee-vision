...
baseLineY = 23.13409616  #taken from extrinsics T vector [ -1.1517719701854450e+01, 2.3134096167196148e+01,-1.0639161017443953e+01 ]
baseLineX = 11.51771970  #taken from extrinsics T vector [ -1.1517719701854450e+01, 2.3134096167196148e+01,-1.0639161017443953e+01 ]
# rgb_uv_f = 1043.2611288935245/2.0  #taken form RGB->UV Q matrix
rgb_uv_fx = 711.89660247266033   # taken from extrinsics 
rgb_uv_fy = 708.38668843513199   # taken from extrinsics 
def getDisparityMap(depthMap):

    base_y = baseLineY * rgb_uv_fy 
    base_x = baseLineX * rgb_uv_fx  
    ...
    dispMap = np.zeros((2,resY,resX), dtype=np.float)
    ...
    for y in range(0, resY, 1):
        for x in range(0, resX, 1):
            if (depthMap[y][x] > 0.2):
                ...
                if (Z < 1500.0):
                    dispMap[0][y][x] = base_x / Z  #in pixels (in theory ) disp in Y
                    dispMap[1][y][x] = base_y / Z  #in pixels (in theory ) disp in Y
...
def getUVTexture(back, front ,mask, dispMap):
    for y in range(0, resY, 1):
        for x in range(0, resX, 1):
            if (mask[y][x] > 0):
                if ( 0 <= (x - int(round(dispMap[0][y][x])) * scX ) < resX) and ( 0 <=  (y + int(round(dispMap[1][y][x]))* scY)  < resY):
                    front[y][x][0] = front[y + int(round(dispMap[1][y][x]) * scY )] [x - int(round(dispMap[0][y][x]) * scX )][0]  #
                    front[y][x][1] = front[y + int(round(dispMap[1][y][x]) * scY )] [x - int(round(dispMap[0][y][x]) * scX )][1]
                    front[y][x][2] = front[y + int(round(dispMap[1][y][x]) * scY )] [x - int(round(dispMap[0][y][x]) * scX )][2]  
