...
# TODO IMPORTANTE !!!! 
# TODO separar depth registration en otro hilo (y volver a medir tiempos)
# TODO bug right camera cut matrix
# TODO poner filtros nuevos y volver a calibrar. 
...
from include.cuda_stream28Stereo import cuStream
from include.cuda_stream28Stereo import Matrices
...
Rtmul = 1.0 #1.429222
err = 0.0 #DEBUG
alphaValue = 202    #255=100% / 0=0% / 128=50%  
fpsCount = 0
...
rCam = normalCamThread(resX, resY, 0, 90)  #right rgb stereo camera
cCam = normalCamThread(resX, resY, 2, 90)  #center camera (UV / INFRA sensor)
...
gpu_right = cuStream(stream_right, resX, resY, alphaValue, matrices_left, matrices_right, base_rgb_left, base_rgb_right, clipping_distance, 'h', True, event_right, 'r')
gpu_right.start()

gpu_left = cuStream(stream_left, resX, resY, alphaValue, matrices_left, matrices_right, base_rgb_left, base_rgb_right, clipping_distance, 'h', False, event_left, 'l')
gpu_left.start()
##################### THREADING CONTROL VARIABLES ##################################
...
try:
    ...
    while True:
        ...
        gpu_left.updateImages(color_image_left, sensor_image_left, depth_image_left, 'l')   #left left OK
        event_left.set()
        gpu_right.updateImages(color_image_right, sensor_image_right, depth_image_right, 'r')  #right right BAD
        event_right.set()
        ...
        if gpu_left.is_alive() == False:
            print('GPU Left thread is death')
            break
        if gpu_right.is_alive() == False:
