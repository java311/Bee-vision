...
try:
    while(cap.isOpened() and cap.isOpened() ):
        ...
        ret, infracam = cap.read()  #cam with leds
        ret2, uvcam =  cap2.read() #cam with no leds
        ...
        cv2.imshow('UV cam', uvcam)
        cv2.imshow('INFRA cam',  infracam)
        cv2.imshow('RGB RS',  rs_color_image)

        #save the images 
        key = cv2.waitKey(23)
        if key & 0xFF == ord(' '):
            cv2.imwrite("captures/cam_uv%02d.png" % count, uvcam)
            cv2.imwrite("captures/cam_infra%02d.png" % count, infracam) 
