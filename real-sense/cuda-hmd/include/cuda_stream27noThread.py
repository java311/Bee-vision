...
class PinnedMem(object):
    # array = np.empty((1,1,1), dtype=np.uint8)
    # array = None
    def __init__(self, size, dtype=np.uint8):
        self.array = np.full(size,0,dtype=dtype)
...
class cuStream():
    ...
    def __init__(self, stream, resX, resY, matrices, base_rgb_uv, clip_dist, orientation, add, side):
        # GPU MATRICES INIT
        self.stream = stream
        self.resX = resX
        self.resY = resY
        ...
        self.cuDisp.upload(np.full((resY, resX), 0, dtype="float32"), stream=self.stream)  #this static init may cause a bug
        # self.cuMapx1_uv = cv2.cuda_GpuMat(); 
        self.cuMapx1_uv.upload(matrices.mapx1_uv, stream=self.stream)  #these are static
        # self.cuMapy1_uv = cv2.cuda_GpuMat(); 
        self.cuMapy1_uv.upload(matrices.mapy1_uv, stream=self.stream)  #these are static
        # self.cuMapx2_uv = cv2.cuda_GpuMat();  
        self.cuMapx2_uv.upload(matrices.mapx2_uv, stream=self.stream)  #these are static
        # self.cuMapy2_uv = cv2.cuda_GpuMat();  
        self.cuMapy2_uv.upload(matrices.mapy2_uv, stream=self.stream)  #these are static

        # # PINNED MEMORY UPLOAD !!
        # pin_mapx1_uv  = PinnedMem(matrices.mapx1_uv.shape,matrices.mapx1_uv.dtype)
        # pin_mapx1_uv.array = matrices.mapx1_uv.copy()
        # self.cuMapx1_uv.upload(pin_mapx1_uv.array, stream=self.stream)  #these are static

        # pin_mapy1_uv  = PinnedMem(matrices.mapy1_uv.shape,matrices.mapy1_uv.dtype)
        # pin_mapy1_uv.array = matrices.mapy1_uv.copy()
        # self.cuMapy1_uv.upload(pin_mapy1_uv.array, stream=self.stream)  #these are static

        # pin_mapx2_uv  = PinnedMem(matrices.mapx2_uv.shape,matrices.mapx2_uv.dtype)
        # pin_mapx2_uv.array = matrices.mapx2_uv.copy()
        # self.cuMapx2_uv.upload(pin_mapx2_uv.array, stream=self.stream)  #these are static

        # pin_mapy2_uv  = PinnedMem(matrices.mapy2_uv.shape,matrices.mapy2_uv.dtype)
        # pin_mapy2_uv.array = matrices.mapy2_uv.copy()
        # self.cuMapy2_uv.upload(pin_mapy2_uv.array, stream=self.stream)  #these are static
        ...
        self.bases.upload(np.full((resY, resX), self.base_rgb_uv, dtype="float32"), stream=self.stream)    #static
        ...
        self.cuRecColor.upload(np.full((resY, resX), 0, dtype="float32"), stream=self.stream)
        # self.cuRecUv = cv2.cuda_GpuMat()
        self.cuRecUv.upload(np.full((resY, resX, 4), 0, dtype="float32"), stream=self.stream)
        # self.cuRecMask = cv2.cuda_GpuMat()  
        self.cuRecMask.upload(np.full((resY, resX), 0, dtype="float32"), stream=self.stream)
        # self.cuResult = cv2.cuda_GpuMat()  
        self.cuResult.upload(np.full((resY, resX, 4), 0, dtype="float32"), stream=self.stream)
        # self.cuFinal = cv2.cuda_GpuMat()  
        self.cuFinal.upload( np.full((resY, resX, 4), 0, dtype="float32"), stream=self.stream)
    ...
    def updateImages(self, color_image, uv_image, depth_image):
        ...
        # self.bases.upload(np.full((self.resY, self.resX), base_rgb_uv, dtype="float32"), stream=self.stream)
        # self.cuMapx1_uv.upload(matrices.mapx1_uv, stream=self.stream)  #these are static
        # self.cuMapy1_uv.upload(matrices.mapy1_uv, stream=self.stream)  #these are static
        # self.cuMapx2_uv.upload(matrices.mapx2_uv, stream=self.stream)  #these are static
        # self.cuMapy2_uv.upload(matrices.mapy2_uv, stream=self.stream)  #these are static
    ...
    def run(self):
        ...
        self.cuDepth.upload(self.depth_image.astype(np.float32), stream=self.stream)  #CHECK THIS, THE CAST WAS ORIGINALLY AFTER THE REMAP
        ...
        self.cuColor.upload(self.color_image, stream=self.stream)
        ...
        # pinchi1 =  self.cuRecColor.download(stream=self.stream)
        # pinchi2 =  self.cuRecUv.download(stream=self.stream)
        # pinchi3 =  self.cuRecMask.download(stream=self.stream)
        ...
        self.recMask = self.cuRecMask.download(stream=self.stream)
        resu = self.cuResult.download(stream=self.stream)
        resu[:, :, 3] = cv2.convertScaleAbs(self.recMask, alpha=255)
        self.cuResult.upload(resu, stream=self.stream)
        ...
        final = cv2.convertScaleAbs(self.cuFinal.download(stream=self.stream))
        ...
        cv2.imshow('final-'+str(self.side), final)
        key = cv2.waitKey(1)
        ...
        if key & 0xFF == ord('q'):
            return -1    #return -1 to exit main loop
        # finally:
