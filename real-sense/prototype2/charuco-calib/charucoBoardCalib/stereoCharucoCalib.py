...
PATH = 'C:\\bee-vision\\source\\real-sense\\prototype2\\charuco-calib\\capture\\captures_1005\\'
...
for i in range(0,len(cam1_images),1):
    ...
    if (response1 + response2 > 60 and response1 == response2):
...
R1, R2, P1, P2, Q, vRoi1, vRoi2 = cv2.stereoRectify(camMat1, dCoeffs1,camMat2, dCoeffs2, (RES_X, RES_Y), R, T,cv2.CALIB_ZERO_DISPARITY, 1, (RES_X, RES_Y))

#save extrinsics matrices of both cameras 
filename = PATH + 'extrinsics.yml'
fs = cv2.FileStorage(filename, cv2.FILE_STORAGE_WRITE)
...
mapx1, mapy1 = cv2.initUndistortRectifyMap(camMat1, dCoeffs1, R1, P1,(RES_X, RES_Y), cv2.CV_16SC2)
...
folder = PATH + 'rectify' #make directory for rectify output
if not os.path.exists(folder):
    os.mkdir(folder)
for i in range(0,len(cam1_images),1):
    up = cv2.imread(cam1_images[i])
    down = cv2.imread(cam2_images[i])

    recUp = numpy.full_like(up, 0, dtype="uint8")
    recDown = numpy.full_like(down, 0, dtype="uint8")

    recUp = cv2.remap(up, mapx1, mapy1, cv2.INTER_LINEAR)
    recDown = cv2.remap(down, mapx2, mapy2, cv2.INTER_LINEAR) 

    # recUp = cv2.rectangle(recUp, (vRoi1[0], vRoi1[1]) , (vRoi1[0]+vRoi1[2], vRoi1[1]+vRoi1[3]), (0, 0, 255),2)
    # recDown = cv2.rectangle(recDown, (vRoi2[0], vRoi2[1]) , (vRoi2[0]+vRoi2[2], vRoi2[1]+vRoi2[3]), (0, 0, 255),2)    

    recUp = cv2.rectangle(recUp, (Vroi[0],Vroi[1]), (Vroi[2],Vroi[3]), (0,0,255), 2)
    recDown = cv2.rectangle(recDown, (Vroi[0],Vroi[1]), (Vroi[2],Vroi[3]), (0,0,255), 2)

    # for (j = 0; j < canvas.rows; j += 16)
    #             line(canvas, Point(0, j), Point(canvas.cols, j), Scalar(0, 255, 0), 1, 8);

    final = cv2.vconcat([recUp, recDown])
